...
public interface PaymentJobRepository extends CrudRepository<PaymentJob, Integer> {
    ...
    @Query(value = "SELECT p.*,pc.* from payments p INNER JOIN payments_customer pc on pc.payment_id=p.id_payment inner join customers c on c.id_customer=pc.customer_id where id_payment = (SELECT payment_id from jobs_payments where job_id = :idJob)", nativeQuery = true)
    PaymentJob findPaymentByJobId(Integer idJob);
}
