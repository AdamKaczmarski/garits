...
SELECT id_role FROM roles where role_name="RECEPTIONIST";
...
DELETE FROM users where id_user IN (1,22);
DELETE FROM users_roles where user_id IN (1,22);
select id_user from users_roles RIGHT join users u on u.id_user=users_roles.user_id where user_id is null;
